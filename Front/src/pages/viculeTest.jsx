...
import { faExclamationTriangle, faInfoCircle, faSort, faSortUp, faSortDown, faEye } from '@fortawesome/free-solid-svg-icons';
...
const VehiculeTable = () => {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      console.error("Erreur fetchVehicles:", error.response?.data || error);
    }
  };
  ...
  const fetchConducteurs = async () => {
    try {
      ...
      console.error("Erreur fetchConducteurs:", error.response?.data || error);
    }
  };
  ...
  const fetchGroupes = async () => {
    try {
      ...
      console.error("Erreur fetchGroupes:", error.response?.data || error);
    }
  };
  ...
  const validateDocuments = () => {
    ...
    // Vérification qu'au moins un document est fourni lors de la création
  ...
  return (
    <div className={`relative min-h-screen transition-all duration-300 ${isModalOpen ? 'overflow-hidden' : ''}`}>
      {/* Arrière-plan flou lorsque la modale est ouverte */}
      ...
      {/* Modale pour afficher les documents (style de DriverTable) */}
      {isModalOpen && (
            ...
            <div className="flex justify-center items-center w-full h-[300px] bg-gray-100 rounded-lg border border-gray-200">
              {modalDocuments.carteGrise.endsWith('.pdf') ? (
                ...
                <img
                  ...
                  onError={(e) => {
                    console.error("Erreur de chargement de l'image Carte Grise:", modalDocuments.carteGrise);
                    e.target.style.display = 'none';
            ...
            <div className="flex justify-center items-center w-full h-[300px] bg-gray-100 rounded-lg border border-gray-200">
              {modalDocuments.attestationAssurance.endsWith('.pdf') ? (
                ...
                <img
                  ...
                  className="max-w-full max-h-full object-contain rounded-lg"
                  onError={(e) => {
                    console.error("Erreur de chargement de l'image Attestation d'Assurance:", modalDocuments.attestationAssurance);
                    e.target.style.display = 'none';
                    e.target.parentElement.innerHTML = '<span class="text-red-500">Erreur de chargement de l\'image</span>';
                  }}
                />
...
// Ajouter l'animation pour la modale (identique à DriverTable)
const styles = `
  @keyframes modalOpen {
    from {
      opacity: 0;
